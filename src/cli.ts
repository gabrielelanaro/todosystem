...
class TodoSystemCLI {
  ...
  async ensureTodoDir(): Promise<void> {
    try {
  ...
  async addTodo(content: string, priority: TodoPriority = 'medium'): Promise<void> {
...
program
  ...
  .action(async (content: string, options: { priority: TodoPriority }) => {
...
program
  ...
  .action(async (id: string, options: { content?: string; status?: TodoStatus; priority?: TodoPriority }) => {
